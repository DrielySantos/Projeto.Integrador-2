...
HTML;
    ...
    $email_message = <<<TXT
Olá!
Uma nova mensagem foi enviada para o site {$c['sitename']}.
 • Data: {$now}
 • Remetente: {$name}
 • E-mail: {$email}
 • Assunto: {$subject}
...
$form_html = <<<HTML
    <form action="/?contacts" method="post" name="contacts" id="contacts">
        ...
        <p>Preencha todos os campos abaixo para enviar uma mensagem para a equipe do <strong>Absoluta&Bela</strong>.</p>
    
        <p>
            <label for="name">Nome:</label>
            <input type="text" name="name" id="name" placeholder="Seu nome e sobrenome" required minlength="3" value="{$name}">
        </p>
    
        <p>
            <label for="email">E-mail:</label>
            <input type="email" name="email" id="email" placeholder="Seu melhor e-mail" required value="{$email}">
        </p>
    
        <p>
            <label for="subject">Assunto:</label>
            <input type="text" name="subject" id="subject" placeholder="Assunto da mensagem" required minlength="5" value="{$subject}">
        </p>
